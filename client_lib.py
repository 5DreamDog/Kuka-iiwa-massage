...
class kuka_iiwa_ros_client:
    ...
    def JointPosition_callback(self, data):
        ...
        values_str = parts[0][1:]
        self.JointPosition = ([float(x) for x in values_str.split(',')], float(parts[1]))
    ...
    def ToolPosition_callback(self, data):
        ...
        parts = data.data.split(']')
        values_str = parts[0][1:]
        self.ToolPosition = ([float(x) for x in values_str.split(',')], float(parts[1]))
    ...
    def ToolForce_callback(self, data):
        ...
        parts = data.data.split(']')
        values_str = parts[0][1:]
        self.ToolForce = ([float(x) for x in values_str.split(',')], float(parts[1]))
    ...
    def ToolTorque_callback(self, data):
        ...
        parts = data.data.split(']')
        values_str = parts[0][1:]
        self.ToolTorque = ([float(x) for x in values_str.split(',')], float(parts[1]))
    ...
    def isReadyToMove_callback(self, data):
        ...
        d = str(data.data).split()
        #print(f"Debug: isReadyToMove: raw='{data.data}'),d[0]='{d[0]}', lower='{d[0].lower()}'")
        stat = d[0].lower() == 'true'
    ...
    def JointAcceleration_callback(self, data):
        ...
        parts = data.data.split(' ')
        self.JointAcceleration = (float(parts[0]), float(parts[1]))
